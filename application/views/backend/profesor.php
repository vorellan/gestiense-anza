...
function grid<?=$nombreTab?>Reload(){//@
	...
	//@
	jQuery("#grid<?=$nombreTab?>").setGridParam({url:"index.php?/<?=$controller?>/grid/<?=$keyTab?>",postData:{"livesearch":true,"f_id":f_id,"f_nombre":f_nombre},page:1}).trigger("reloadGrid");
}
//////////////////////
<?php } ?>
...
function fk<?=$nombreTab?>Default(){//NO TOCAR
	$('#form<?=$nombreTab?>Standard')[0].profesor_id.value=<?=$keyTab?>;//@
	$('#form<?=$nombreTab?>Standard')[0].profesor_id_aux.value=<?=$keyTab?>;//@
	$('#ib_profesor_id').hide();
	$('#clean_profesor_id').hide();
}
///////
<?php } ?>
