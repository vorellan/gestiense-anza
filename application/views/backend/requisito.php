...
function grid<?=$nombreTab?>Reload(){//@
	...
	//@
	jQuery("#grid<?=$nombreTab?>").setGridParam({url:"index.php?/<?=$controller?>/grid/<?=$keyTab?>",postData:{"livesearch":true,"f_id":f_id,"f_subtema_id":f_subtema_id},page:1}).trigger("reloadGrid");
}
//////////////////////
<?php } ?>
...
$('#ib_subtema_id').click(function()
	{
		$.ajax({
			type: "POST",
			url: 'index.php?/curso_controller/cargaModulo/IBSubtema/subtema_id',
			success: function(datos){
...
function fk_subtema_id(valor){//@
	...
	$('#form<?=$nombreTab?>Standard')[0].subtema_id_aux.value=valor+fkName('IBSubtema',valor,2);//@ 
	$("#ib").hide();
}
