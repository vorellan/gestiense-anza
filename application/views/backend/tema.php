...
<div id="dataForm<?=$nombreTab?>" style="display:none">
	...
	<table  class="tabla_form" >
		...
		<tr>
			<th>CURSO ID</th>
			<td><input type="hidden" id="curso_id" name="curso_id" value="" ><input type="text" size="30" id="curso_id_aux" name="curso_id_aux" value="" class="required readonly"  readonly ></input>

			<span id="ib_curso_id" class="ibButton"></span>
			<span id="clean_curso_id" class="cleanButton" onclick="javascript:limpiar('curso_id');limpiar('curso_id_aux')"></span></td>
		</tr>
		<tr>
			<th>NOMBRE</th>
...
var mygrid<?=$nombreTab?>=jQuery("#grid<?=$nombreTab?>").jqGrid({
    ...
    width:900,
    height:'auto',
    colNames:['ID','CURSO_ID','NOMBRE'],//@
    colModel:[//@
        {name:'id',index:'id', width:60},
		{name:'curso_id',index:'curso_id', width:60},
...
$("#gs_curso_id").attr("id","f_curso_id<?=$nombreTab?>");
...
function grid<?=$nombreTab?>Reload(){//@
	...
	var f_curso_id = jQuery("#f_curso_id<?=$nombreTab?>").val();
	...
	//@
	jQuery("#grid<?=$nombreTab?>").setGridParam({url:"index.php?/<?=$controller?>/grid/<?=$keyTab?>",postData:{"livesearch":true,"f_id":f_id,"f_curso_id":f_curso_id,"f_nombre":f_nombre},page:1}).trigger("reloadGrid");
}
//////////////////////
<?php } ?>
...
function form<?=$nombreTab?>Edit(id){
	...
	$.ajax({
		...
		success: function(data){
			...
			$('#form<?=$nombreTab?>Standard')[0].curso_id.value=json.curso_id;
			$('#form<?=$nombreTab?>Standard')[0].curso_id_aux.value=json.curso_id+' ['+json.curso_id_aux+']';
...
<?php if(!$inputBuscador){?>
//CALL INPUT BUSCADOR 
$('#ib_curso_id').click(function()
	{
		$.ajax({
			type: "POST",
			url: 'index.php?/curso_controller/cargaModulo/IBCurso/curso_id',
			success: function(datos){
...
function fk_curso_id(valor){//@
	$('#form<?=$nombreTab?>Standard')[0].curso_id.value=valor;//@
	$('#form<?=$nombreTab?>Standard')[0].curso_id_aux.value=valor+fkName('IBCurso',valor,3);//@ 
	$("#ib").hide();
}
