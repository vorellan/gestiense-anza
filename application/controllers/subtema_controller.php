...
class Subtema_controller extends Controller {//@
    private $data;
	private $myModel='Subtema_model';//@
    
    function Subtema_controller(){//@
        parent::Controller();
	...
	function cargaModulo($nombreTab='',$inputBuscador=false,$keyTab='',$keyTab='',$keyTab=''){//******************@ AGREGAR TANTOS KEYTAB COMO FK TENGA LA TABLA
		...
		$this->data['controller']="subtema_controller";//@lowercase
		$this->data['nombreModulo']="Subtema";//@
        $this->load->view('backend/subtema',$this->data);//@
    }
	...
	function grid($keyTab=''){
		...
		$o->exigencia=(isset($_REQUEST['f_exigencia']))?$_REQUEST['f_exigencia']:'';
	...
	function insertar(){
		...
		$o->exigencia=$_REQUEST['exigencia'];
