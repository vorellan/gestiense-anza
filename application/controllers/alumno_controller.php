...
class Alumno_controller extends Controller {//@
    private $data;
	private $myModel='Alumno_model';//@
    
    function Alumno_controller(){//@
        parent::Controller();
	...
	function cargaModulo($nombreTab='',$inputBuscador=false,$keyTab=''){//@ AGREGAR TANTOS KEYTAB COMO FK TENGA LA TABLA
		...
		$this->data['controller']="alumno_controller";//@lowercase
		$this->data['nombreModulo']="Alumno";//@
        $this->load->view('backend/alumno',$this->data);//@
    }
	...
	function grid($keyTab=''){
		...
		$o->user=(isset($_REQUEST['f_user']))?$_REQUEST['f_user']:'';
