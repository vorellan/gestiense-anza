...
class Asignatura_controller extends Controller {//@
    private $data;
	private $myModel='Asignatura_model';//@
    
    function Asignatura_controller(){//@
        parent::Controller();
	...
	function cargaModulo($nombreTab='',$inputBuscador=false,$keyTab='',$keyTab='',$keyTab=''){//******************@ AGREGAR TANTOS KEYTAB COMO FK TENGA LA TABLA
		...
		$this->data['controller']="asignatura_controller";//@lowercase
		$this->data['nombreModulo']="Asignatura";//@
        $this->load->view('backend/asignatura',$this->data);//@
    }
	...
	function grid($keyTab=''){
		...
		$o->tipo_curso=(isset($_REQUEST['f_tipo_curso']))?$_REQUEST['f_tipo_curso']:'';
	...
	function insertar(){
		...
		$o->tipo_curso=$_REQUEST['tipo_curso'];
