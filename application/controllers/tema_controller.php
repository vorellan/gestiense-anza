...
class Tema_controller extends Controller {//@
    private $data;
	private $myModel='Tema_model';//@
    
    function Tema_controller(){//@
        parent::Controller();
	...
	function cargaModulo($nombreTab='',$inputBuscador=false,$keyTab='',$keyTab='',$keyTab=''){//******************@ AGREGAR TANTOS KEYTAB COMO FK TENGA LA TABLA
		...
		$this->data['controller']="tema_controller";//@lowercase
		$this->data['nombreModulo']="Tema";//@
        $this->load->view('backend/tema',$this->data);//@
    }
	...
	function grid($keyTab=''){
		...
		$o->curso_id=(isset($_REQUEST['f_curso_id']))?$_REQUEST['f_curso_id']:'';
