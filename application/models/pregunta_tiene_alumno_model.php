...
class Pregunta_tiene_alumno_model extends CI_Model {//@

	function Pregunta_tiene_alumno_model(){
		parent::CI_Model();
	...
	function grid($p,$o){
		...
		if($o->pregunta_id!='') $where.= " AND a.pregunta_id LIKE '$o->pregunta_id%' ";
		if($o->alumno_id!='') $where.= " AND a.alumno_id LIKE '$o->alumno_id%' ";
		if($o->respuesta!='') $where.= " AND a.respuesta LIKE '$o->respuesta%' ";
		

		//FIN BLOQUE FILTRO
		
		//PAGINADOR
		$queryCount="SELECT COUNT(*) AS count FROM pregunta_tiene_alumno as a ".$where;//@
		...
		//@
		$queryData = "SELECT a.*,e.titulo as nombrePregunta,p.id as nombreAlumno
		FROM pregunta_tiene_alumno as a,pregunta as e,alumno as p ".$where." 
		and e.id=a.pregunta_id and p.id=a.alumno_id ORDER BY $p->sidx $p->sord LIMIT $p->limit offset $start "; 
        ...
        if ($result->num_rows() > 0)
		{
			...
			foreach($result->result() as $rs){//@
				...
				$pregunta=" [".$rs->nombrePregunta."]";
				$alumno=" [".$rs->nombreAlumno."]";
				$response->rows[$i]['cell']=array($rs->id,$rs->pregunta_id.$pregunta,$rs->alumno_id.$alumno,$rs->respuesta);  
				$i++;
			}
