...
class Tema_model extends CI_Model {//@
	...
	function grid($p,$o){
		...
		if($o->curso_id!='') $where.= " AND a.curso_id LIKE '$o->curso_id%' ";
		...
		//PAGINADOR
		$queryCount="SELECT COUNT(*) AS count FROM tema as a ".$where;//@
		...
		//@
		$queryData = "SELECT a.*,p.nombre as nombreCurso
		FROM tema as a,curso as p ".$where." 
		 and
		  p.id=a.curso_id  ORDER BY $p->sidx $p->sord LIMIT $p->limit offset $start "; 
        ...
        if ($result->num_rows() > 0)
		{
			...
			foreach($result->result() as $rs){//@
				...
				$curso=" [".$rs->nombreCurso."]";
				$response->rows[$i]['cell']=array($rs->id,$rs->curso_id.$curso,$rs->nombre);  
				$i++;
			}
	...
	function insertar($o){
		...
		$result=$this->db->insert("tema",$query);//@
	...
	function editar($key,$o){
		...
		$result=$this->db->update("tema",$query);//@
	...
	function verEntidad($key){
        ...
        $query = "SELECT a.id,a.curso_id,a.nombre,a.profesor_id,
		p.nombre as nombreCurso
		FROM tema as a, curso as p
		WHERE a.id='$key' and p.id=a.curso_id";
        ...
        foreach($result->result() as $rs)
		{
			$o=new stdClass();//@****************************************************+
			$o->id=$rs->id;
			$o->curso_id=$rs->curso_id;
			$o->curso_id_aux=$rs->nombreCurso;
			$o->nombre=$rs->nombre;
